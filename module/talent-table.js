...
export class TalentTableForm extends FormApplication {
    ...
    async _initTable() {
        ...
        for (var j = 0; j < 6; ++j) {
            ...
            var item = { title: await game.i18n.localize(`INSANE.${name}${i}`), 
                         id: `col-${j}-${i}` ,
                         value: (this.reset) ? "" : game.settings.get("insane", `INSANE.${name}${i}`)
                       }
    ...
    async _updateObject(event, formData) {
        ...
        for (var j = 0; j < 6; ++j) {
            let item = $(`#col-${j}-${i}`)[0];
            var name = String.fromCharCode(65 + j);
            game.settings.set("insane", `INSANE.${name}${i}`, item.value);
        }
    }
