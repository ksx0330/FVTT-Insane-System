...
export class SecretJournalSheet extends JournalSheet {
    ...
    async _showSecret(ev) {
        try {
            var id = this.object.data.flags["insane"].secret.journal._id;
            var mode = this.object.data.flags["insane"].secret.mode;
...
class SetDialog extends Dialog {
    ...
    async _submit() {
        ...
        if (secret == undefined) {
            await this.journal.unsetFlag("insane", "secret");
            return;
        }

        var data = {"journal": secret, "mode": mode };
        await this.journal.setFlag("insane", "secret", data);
