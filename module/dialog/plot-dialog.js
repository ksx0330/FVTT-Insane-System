export class PlotDialog extends Dialog {
  ...
  async _ready(event) {
    ...
    let chatData = {"content": (this.ready) ? game.i18n.localize("INSANE.ReadyPlot") : game.i18n.localize("INSANE.AwaitPlot"), "speaker": ChatMessage.getSpeaker({ alias: this.name })};
    ChatMessage.create(chatData);
  
    if (game.user.id === this.receiver) {
      let plot = game.insane.plot.find(a => (a.actorId === this.actorId && a.combatant === this.combatantId));
      ...
      game.socket.emit("system.insane", {id: "resp", sender: game.user.id, receiver: this.receiver, data: { actorId: this.actorId, combatantId: this.combatantId, dice: this.plot, ready: this.ready } });
